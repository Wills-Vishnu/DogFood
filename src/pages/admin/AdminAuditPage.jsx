...
export const AdminAuditPage = ({ currentUser }) => {
  const [logs, setLogs] = useState([]);
  const [loading, setLoading] = useState(true);
  const [searchTerm, setSearchTerm] = useState('');

  const user = currentUser || mockUsers.admin;
  ...
  return (
    <MainLayout currentUser={currentUser}>
      ...
      <div className="mb-8">
        <h1 className="text-4xl font-bold text-brand-900 mb-2">System Audit Log</h1>
        <p className="text-lg text-brand-600">Global system activity</p>
      </div>
                ...
                <div key={log.id} className="p-4 border border-brand-200 rounded-lg hover:bg-brand-50">
                  <div className="flex justify-between items-start mb-2">
                    <div>
                      ...
                      <p className="text-xs text-brand-600 mt-1">By: {log.actor?.name}</p>
                    </div>
